...
/*
  if ($calls.length) {
    ...
    ;
    // Hide results since no call has been done yet.
    $('#brapi_query_result_title, #brapi_query_results')
      .not('.brapi-processed')
      .addClass('brapi-processed')
      .hide();
    // By default, hide all the "call" forms to only display the selected one.
    // Add "call" selector to the page.
    $('#brapi_query_calls')
      .find('h4')
      .first()
      .after(
        $('<caption class="brapi-call-select">Select a BrAPI call: </caption>')
        .append($select)
      );
